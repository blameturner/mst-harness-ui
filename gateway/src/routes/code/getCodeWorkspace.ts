...
import { getCodeWorkspace as harnessGetCodeWorkspace } from '../../services/harness/index.js';
import { findOwnedCodeConversation } from './findOwnedCodeConversation.js';

export async function getCodeWorkspace(c: Context) {
  ...
  try {
    ...
    const res = await harnessGetCodeWorkspace(conversationId, Number(orgId));
