...
export function getEmailWidget() {
  return http.get('api/home/widgets/email').json<WidgetEnvelope<null>>();
}

export function getCalendarWidget() {
  return http.get('api/home/widgets/calendar').json<WidgetEnvelope<null>>();
}
...
export function getGraphWidget(opts: { orgId?: number; limit?: number } = {}) {
  ...
  return http
    .get('api/home/widgets/graph', { searchParams: { org_id: orgId, limit } })
...
export function getActivityWidget(opts: { orgId?: number; limit?: number } = {}) {
  ...
  const limit = opts.limit ?? 10;
  return http
    .get('api/home/widgets/activity', { searchParams: { org_id: orgId, limit } })
